...
const createCourse = async (req, resp) => {  
  ...
  try {
    ...
    const instructorArray = JSON.parse(req.body.instructor);
    const values = instructorArray.map((instructor) => instructor.value);
    ...
    if (usermail) {
      ...
      const courseInstances = values.map((instructor) => ({
        img,
        title,
        desc,
        level,
      ...
      let result = await CourseSchema.insertMany(courseInstances);
      resp.status(200).json({
        code: 200,
...
const putCourse = async (req, res) => {  
  try {
    ...
    const instructorArray = JSON.parse(req.body.instructor);
    const instructors = instructorArray.map((instructor) => instructor.value);
    let status = req.body.status;


    const courseInstances = instructors.map((instructor) => ({
      img,
      title,
      desc,
      level,
